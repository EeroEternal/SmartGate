...
- **表单控件高度与尺寸一致**：并排或同一表单中的输入框（`input`）与下拉框（`Select`）在尺寸等级（如 `sm` 或 `md`）上必须保持一致，高度必须对齐（如 `sm` 尺寸统一使用 `h-9` / `py-1.5` 与 `rounded-md`），禁止出现输入框和下拉框高低不一的情况。
...
- **卡片与标签文本完整显示**：卡片、Badge 或 Label 在展示模型名称、提供商及关键标识时，必须确保文本清晰完整可读，严禁因超长前缀或宽度不足导致关键模型名或信息被截断成 `...` 无法辨认。
- **超长字段优化策略**：当字段（如带有 provider 前缀的 `provider/model-name`、复杂 URL 或哈希 ID）长度超出卡片容器时，必须采用合理方式优化展示：
  1. **提取核心字段并分级展示**：将主标题精简为核心标识（如 `claude-3-5-sonnet`），将附属信息（如 Provider、Protocol）拆分提取为独立的微型徽章（Tag/Badge）或放在卡片次要位置；
  2. **悬停 Tooltip 兜底**：必须保留 `title="..."` 属性或自定义 Tooltip，保证鼠标悬停时始终可以查看完整原始文本；
  3. **布局自适应换行或分行**：对于长文本优先使用自然换行（`break-words`），避免生硬的单行强制 `truncate` 导致有效字符丢失。
...
- 弹窗、卡片和说明中的代码示例（curl、CLI 命令、配置片段等）**不得出现横向滚动条**。应通过自动换行（`whitespace-pre-wrap` / `break-words`）、缩短示例内容或调整布局，确保内容在容器宽度内完整可见。
- 长密钥、Token 或 URL 等单行内容应使用 `break-all` 或 `break-words` 自动换行，避免撑破容器或触发滚动条。
- 代码块优先使用 `pre` + `code` 组合，并通过 Tailwind 工具类控制换行，而不是依赖 `overflow-x-auto` 隐藏溢出内容。

#### 数值与价格显示精度规范

- **价格与价值数值格式化**：所有展示给用户的模型价格（如 `$/1M tokens`）、花费金额与模型价值等浮点数值，展示时统一保留 2 位小数（如 `$0.14`、`$1.20`）；表单输入框和配置项中格式化为**最多保留小数点后 4 位**且去除多余尾随零（如 `3.6`、`0.0014`），严禁在界面输入框或标签上直接输出 `3.5999999999999996` 等未处理的长浮点数。
- **能力分与综合得分**：能力综合评分统一格式化为 2 位小数（如 `0.85`）。
