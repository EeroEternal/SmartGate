...
1. **画像层**：当 Pool 内存在「配置分差 < 0.05 但家族画像差 ≥ 0.05」的 Endpoint 对时，说明这份配置无法表达强弱，
   整池改用家族画像（Pro 0.92 / Flash 0.65）并告警。Admin UI 展示的也是生效值，并标注 `auto` 说明原始配置值。
   刻意配置的区分度（例如 Flash 0.95 / Pro 0.80）与同家族多账号都不受影响。
2. **排序层**：Endpoint 画像额外携带家族能力分，高难度排序键为
   能力档（配置值，每 0.01 一档）> 家族画像档 > 单次成本，作为画像层之外的兜底。

### 7.5 可观测性

Provider 卡片上的 `Routes hard requests` 直接来自路由打分（`explain` 的首个未被排除候选），
因此它表达的是「当前高复杂度请求实际会去哪」，会随健康、冷却与工具支持变化，而不是「能力分最大」的重复。

Endpoint 健康状态在网关重启后会从数据库回填到内存指标，因此 `degraded` 不会因重启而丢失，
也能被重启后的首次成功请求清除（此前重启会让内存状态回到 healthy，导致数据库里的 `degraded` 永久残留）。
Admin UI 在本进程尚无请求样本时把状态标记为 `Last known ...`，不会用「0 errors out of 0 requests」冒充当前状态。
