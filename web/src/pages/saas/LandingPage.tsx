  ...
  return (
    <div className="min-h-screen bg-zinc-50 text-zinc-950">
      ...
      <main>
        ...
        <section className="bg-zinc-950 text-white"><div className="max-w-6xl mx-auto px-6 py-20 grid lg:grid-cols-[.9fr_1.1fr] gap-12 items-start"><div><div className="flex items-center gap-2 text-sm text-zinc-300"><ShieldCheck className="w-4 h-4" /> Codex-ready gateway</div><h2 className="mt-5 text-3xl font-semibold tracking-tight">Use XGate from Codex GUI.</h2><p className="mt-4 max-w-xl text-sm leading-7 text-zinc-400">Connect Codex to an XGate model service with a dedicated Profile. Keep your base Codex settings unchanged while routing Fusion requests through one API key, with XGate handling provider choice, budgets, and usage tracking.</p><div className="mt-7 space-y-3 text-sm text-zinc-300"><div><span className="font-medium text-white">1.</span> Create a model service and API key in XGate.</div><div><span className="font-medium text-white">2.</span> Add the Fusion Profile and model catalog to <code className="text-zinc-200">~/.codex/</code>.</div><div><span className="font-medium text-white">3.</span> Start Codex with <code className="text-zinc-200">--profile fusion</code>.</div></div></div><div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5 font-mono text-xs leading-7 text-zinc-300"><div className="text-zinc-500"># ~/.codex/fusion.config.toml</div><div>model = <span className="text-emerald-400">"fusion"</span></div><div>model_provider = <span className="text-emerald-400">"xgate"</span></div><div>model_catalog_json = <span className="text-emerald-400">"/path/to/.codex/models.json"</span></div><div className="mt-3">[model_providers.xgate]</div><div>base_url = <span className="text-emerald-400">"https://api.xgate.sh/v1"</span></div><div>wire_api = <span className="text-emerald-400">"chat_completions"</span></div><div className="mt-3 text-zinc-500"># Launch Codex with the Profile</div><div className="text-white">ChatGPT --profile fusion</div><div className="mt-4 border-t border-zinc-800 pt-3 font-sans text-xs text-zinc-500">Use the full setup guide for the models.json schema and troubleshooting.</div></div></div></section>
